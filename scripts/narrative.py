...
def _describe_panns(top_categories: list[dict],
    ...
    v0.12.6 (P4 caso A): aggiunto parametro `profile` per modulare il
...
def _describe_onset_events(window_t_start: float, window_t_end: float,
    ...
    """v0.12.6 (P4 caso A): cita timestamp degli onset puntuali nella
...
def _resolve_narrative_profile(summary: dict, profile_arg: str) -> str:
    """v0.12.6 (P4 caso A): risolve il profilo narrativo richiesto.
...
def build_full_narrative(
    ...
    # v0.12.6 (P4 caso A): risolvi profilo narrativo. Se `auto`, decide
...
def narrative_summary(
    ...
    v0.12.6 (P4 caso A): aggiunto `profile` (acousmatic/didactic/auto).
    """
    if mode == "none":
